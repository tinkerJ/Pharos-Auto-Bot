...
async function main() {
	try {
		...
		const privateKeys = fs
			.readFileSync(path.join(process.cwd(), "pk.txt"), "utf8")
			.split("\n")
		...
		if (userChoice === "1") {
			...
			if (fs.existsSync(proxyFilePath)) {
				...
				if (proxies.length > 0) {
					useProxy = true;
					logger.info(
						`Loaded ${privateKeys.length} private keys and ${proxies.length} proxies`
					);
				} else {
...
async function processBatch(privateKeys, proxies, batchIndex, startIndex) {
	...
	logger.info(`Thread ${threadId}: Starting to process ${privateKeys.length} accounts`, {
		walletIndex: threadLogId,
	});
	...
	for (let i = 0; i < privateKeys.length; i++) {
		...
		logger.info(
			`Thread ${threadId}: Processing account ${i + 1}/${
				privateKeys.length
			} (Global Wallet ID: ${globalWalletIndex})`,
			{ walletIndex: threadWalletId }
		);
		...
		try {
			...
			logger.error(
				`Thread ${threadId}: Error processing account ${i + 1}/${privateKeys.length}: ${
					error.message
				}`,
				{ walletIndex: threadWalletId }
			);
	...
	logger.info(`Thread ${threadId}: Completed processing all accounts`, {
		walletIndex: threadLogId,
	});
}
...
async function processAccount(privateKey, proxy, walletIndex, threadId, globalWalletIndex) {
	...
	let allProxies = useProxy
		? fs
				.readFileSync(path.join(process.cwd(), "proxy.txt"), "utf8")
				.split("\n")
				.filter((line) => line.trim() !== "")
				.map((line) => line.trim())
		: [];
	...
	const rotateProxyIfNeeded = async (error) => {
		if (useProxy && isProxyError(error.message) && allProxies.length > 1) {
			...
			if (newProxy !== proxy) {
				logger.warn(
					`Rotating proxy due to error: ${
						error.message
					}. Switching to proxy: ${newProxy.substring(0, 15)}...`,
					{ walletIndex }
				);
		...
		while (!loginSuccess && loginAttempts < maxLoginAttempts) {
			try {
				...
				if (await rotateProxyIfNeeded(error)) {
					...
					logger.info(
						`Retrying login with new proxy (attempt ${loginAttempts}/${maxLoginAttempts})`,
						{ walletIndex }
					);
				} else if (loginAttempts < maxLoginAttempts) {
					logger.warn(
						`Login failed: ${error.message}. Retrying (attempt ${loginAttempts}/${maxLoginAttempts})`,
						{ walletIndex }
					);
					await sleep(3000 * loginAttempts);
				} else {
					throw new Error(
						`Failed to login after ${maxLoginAttempts} attempts: ${error.message}`
					);
				}
			}
		}
	...
	if (config.tasks.checkin) {
		if (!isAuthenticated) {
			logger.warn(
				`Skipping check-in because not authenticated. Enable login in config to use this feature.`,
				{ walletIndex }
			);
		} else {
			...
			while (!checkinSuccess && checkinAttempts < maxCheckinAttempts) {
				try {
					...
					if (await rotateProxyIfNeeded(error)) {
						...
						socialService.setJwt(jwt);
						logger.info(
							`Retrying check-in with new proxy (attempt ${checkinAttempts}/${maxCheckinAttempts})`,
							{ walletIndex }
						);
					} else if (checkinAttempts < maxCheckinAttempts) {
						logger.warn(
							`Check-in failed: ${error.message}. Retrying (attempt ${checkinAttempts}/${maxCheckinAttempts})`,
							{ walletIndex }
						);
						await sleep(3000 * checkinAttempts);
					} else {
						logger.error(
							`Failed to complete check-in after ${maxCheckinAttempts} attempts: ${error.message}`,
							{ walletIndex }
						);
						break;
					}
	...
	if (config.tasks.faucet.native) {
		if (!isAuthenticated) {
			logger.warn(
				`Skipping native faucet because not authenticated. Enable login in config to use this feature.`,
				{ walletIndex }
			);
		} else {
			try {
	...
	if (config.tasks.social.enabled) {
		if (!isAuthenticated) {
			logger.warn(
				`Skipping social tasks because not authenticated. Enable login in config to use this feature.`,
				{ walletIndex }
			);
		} else {
	...
	if (config.tasks.onchain.self_transfer.enabled) {
		...
		if (isAuthenticated) {
			try {
				...
				remainingCount = Math.min(
					taskConfig.max_count - completedCount,
					taskConfig.max_count
				);
			} catch (error) {
				logger.warn(
					`Could not get task list for self-transfers: ${error.message}. Using max count from config.`,
					{ walletIndex }
				);
			}
		}
		...
		if (remainingCount > 0) {
			...
			for (let i = 0; i < remainingCount; i++) {
				...
				if (taskConfig.random_amount) {
					const randomIndex = Math.floor(
						Math.random() * taskConfig.amount_options.length
					);
				...
				logger.info(
					`Performing self-transfer ${
						i + 1
					}/${remainingCount} with amount ${amount} PHRS...`,
					{ walletIndex }
				);
				...
				if (txHash) {
					if (isAuthenticated) {
						try {
							...
							logger.info(`Self-transfer ${i + 1}/${remainingCount} verified`, {
								walletIndex,
							});
						} catch (error) {
							logger.warn(`Could not verify self-transfer task: ${error.message}`, {
								walletIndex,
							});
						}
					} else {
						logger.info(
							`Self-transfer ${
								i + 1
							}/${remainingCount} completed (not verified - login disabled)`,
							{ walletIndex }
						);
					}
			...
			logger.info(`Already completed all ${taskConfig.max_count} self transfers`, {
				walletIndex,
			});
		}
	}
	...
	if (config.tasks.faucet.usdc || config.tasks.faucet.usdt) {
		if (!isAuthenticated) {
			logger.warn(
				`Skipping token faucet because not authenticated. Enable login in config to use this feature.`,
				{ walletIndex }
			);
		} else {
	...
	if (config.tasks.onchain.swap.enabled) {
		...
		if (isAuthenticated) {
			try {
				...
				logger.warn(
					`Could not get task list for swaps: ${error.message}. Using max count from config.`,
					{ walletIndex }
				);
			}
		}
		...
		if (remainingCount > 0) {
			...
			for (let i = 0; i < remainingCount; i++) {
				...
				logger.info(`Swapping ${pair.amount} ${pair.from} to ${pair.to}...`, {
					walletIndex,
				});
	...
	if (config.tasks.onchain.liquidity.enabled) {
		...
		if (isAuthenticated) {
			try {
				...
				logger.warn(
					`Could not get task list for liquidity: ${error.message}. Using max count from config.`,
					{ walletIndex }
				);
			}
		}
		...
		if (remainingCount > 0) {
			logger.info(`Need to perform ${remainingCount} more liquidity additions`, {
				walletIndex,
			});
			...
			for (let i = 0; i < remainingCount; i++) {
				...
				logger.info(
					`Adding liquidity: ${pair.amount0} ${pair.token0} and ${pair.amount1} ${pair.token1}...`,
					{ walletIndex }
				);
				...
				if (txHash) {
					logger.info(`Liquidity addition ${i + 1}/${remainingCount} completed`, {
						walletIndex,
					});
			...
			logger.info(`Already completed all ${taskConfig.max_count} liquidity additions`, {
				walletIndex,
			});
		}
	}
